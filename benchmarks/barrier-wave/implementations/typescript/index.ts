...
let out!:any;const sa:any[]=[];const _ts:number[]=[];for(let i=-wu;;i++){const st=process.hrtime.bigint();out=await kernel(ps);const el=process.hrtime.bigint()-st;if(i>=0){const ns=Number(el);sa.push({iteration:sa.length+1,kernelTimeNanoseconds:ns});_ts.push(ns);if(_ts.length>=maxIt||(_ts.length>=minIt&&_ciW(_ts)<=targetCi))break}}
