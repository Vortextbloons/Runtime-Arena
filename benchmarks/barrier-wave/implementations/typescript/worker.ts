...
parentPort!.on("message",(msg:{cmd:string;workerId?:number;itemsPerWorker?:number;roundsPerItem?:number;phaseSeed?:number})=>{
  if(msg.cmd==="init"){wId=msg.workerId!;ipw=msg.itemsPerWorker!;rpi=msg.roundsPerItem!}
    ...
    const{phaseSeed}=msg as {phaseSeed:number};let lx=0;let lsLo=0,lsHi=0;
    for(let it=0;it<ipw;it++){const gid=(wId*ipw+it)>>>0;let x=(phaseSeed!^gid^Math.imul(wId,0x9e3779b9)>>>0)>>>0;for(let r=0;r<rpi;r++){x=(x^(x<<13))>>>0;x=(x^(x>>>17))>>>0;x=(x^(x<<5))>>>0;x=(Math.imul(x,0x9e3779b1)+0x85ebca77)>>>0}lx^=x;lsLo=(lsLo+x)|0;if((lsLo>>>0)<(x>>>0))lsHi=(lsHi+1)|0}
