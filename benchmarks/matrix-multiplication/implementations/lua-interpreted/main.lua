...
local function ci_width(samples)
    local n = #samples
    if n < 2 then return huge_check end
    local sum = 0
    for i = 1, n do sum = sum + samples[i] end
    local mean = sum / n
    if mean <= 0 then return huge_check end
    local var = 0
...
if not input_file or not output_file or not timing_output_file then
    io.stderr:write("Usage: lua main.lua --input <input-file> --output <output-file> --timing-output <timing-file>\n"); os.exit(1) end
