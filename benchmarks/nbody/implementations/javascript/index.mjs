...
function kernel(b){
  ...
  const hash=(key)=>createHash("sha256").update(b.flatMap(x=>x[key]).map(x=>x.toFixed(9)+",").join("")).digest("hex");
...
let output;const samples=[];for(let i=-warmups;i<iterations;i++){const state=input.bodies.map(b=>({mass:b.mass,position:[...b.position],velocity:[...b.velocity]}));const start=process.hrtime.bigint();output=kernel(state);const elapsed=process.hrtime.bigint()-start;if(i>=0)samples.push({iteration:i+1,kernelTimeNanoseconds:Number(elapsed)})}
