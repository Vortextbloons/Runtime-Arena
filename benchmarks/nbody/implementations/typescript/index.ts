...
let output;const samples=[];for(let i=-warmups;i<iterations;i++){const state=structuredClone(input.bodies);const start=process.hrtime.bigint();output=kernel(state);const elapsed=process.hrtime.bigint()-start;if(i>=0)samples.push({iteration:i+1,kernelTimeNanoseconds:Number(elapsed)})}
