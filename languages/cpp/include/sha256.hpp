#pragma once
#include <array>
#include <cstdint>
#include <cstring>
#include <string>

class SHA256 {
public:
    SHA256() { reset(); }

    void reset() {
        h_ = {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
              0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};
        datalen_ = 0;
        bitlen_ = 0;
    }

    void update(const uint8_t* data, size_t len) {
        for (size_t i = 0; i < len; i++) {
            data_[datalen_++] = data[i];
            if (datalen_ == 64) {
                transform();
                bitlen_ += 512;
                datalen_ = 0;
            }
        }
    }

    void update(const std::string& s) {
        update(reinterpret_cast<const uint8_t*>(s.data()), s.size());
    }

    std::array<uint8_t, 32> digest() {
        size_t i = datalen_;
        data_[i++] = 0x80;
        if (i > 56) {
            while (i < 64) data_[i++] = 0;
            transform();
            i = 0;
        }
        while (i < 56) data_[i++] = 0;
        bitlen_ += datalen_ * 8;
        for (int j = 7; j >= 0; j--)
            data_[i++] = static_cast<uint8_t>((bitlen_ >> (j * 8)) & 0xff);
        transform();
        std::array<uint8_t, 32> out;
        for (int j = 0; j < 8; j++) {
            out[j*4]   = static_cast<uint8_t>((h_[j] >> 24) & 0xff);
            out[j*4+1] = static_cast<uint8_t>((h_[j] >> 16) & 0xff);
            out[j*4+2] = static_cast<uint8_t>((h_[j] >> 8) & 0xff);
            out[j*4+3] = static_cast<uint8_t>(h_[j] & 0xff);
        }
        return out;
    }

    std::string hex() {
        auto d = digest();
        static const char* hexchars = "0123456789abcdef";
        std::string s;
        s.reserve(64);
        for (auto b : d) {
            s.push_back(hexchars[b >> 4]);
            s.push_back(hexchars[b & 0xf]);
        }
        return s;
    }

private:
    uint32_t rotr(uint32_t x, uint32_t n) { return (x >> n) | (x << (32 - n)); }
    uint32_t ch(uint32_t x, uint32_t y, uint32_t z) { return (x & y) ^ (~x & z); }
    uint32_t maj(uint32_t x, uint32_t y, uint32_t z) { return (x & y) ^ (x & z) ^ (y & z); }
    uint32_t ep0(uint32_t x) { return rotr(x, 2) ^ rotr(x, 13) ^ rotr(x, 22); }
    uint32_t ep1(uint32_t x) { return rotr(x, 6) ^ rotr(x, 11) ^ rotr(x, 25); }
    uint32_t sig0(uint32_t x) { return rotr(x, 7) ^ rotr(x, 18) ^ (x >> 3); }
    uint32_t sig1(uint32_t x) { return rotr(x, 17) ^ rotr(x, 19) ^ (x >> 10); }

    void transform() {
        static const uint32_t k[64] = {
            0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,
            0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,
            0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,0x5cb0a9dc,0x76f988da,
            0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd5a79147,0x06ca6351,0x14292967,
            0x27b70a85,0x2e1b2138,0x4d2c6dfc,0x53380d13,0x650a7354,0x766a0abb,0x81c2c92e,0x92722c85,
            0xa2bfe8a1,0xa81a664b,0xc24b8b70,0xc76c51a3,0xd192e819,0xd6990624,0xf40e3585,0x106aa070,
            0x19a4c116,0x1e376c08,0x2748774c,0x34b0bcb5,0x391c0cb3,0x4ed8aa4a,0x5b9cca4f,0x682e6ff3,
            0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,0x90befffa,0xa4506ceb,0xbef9a3f7,0xc67178f2
        };
        uint32_t w[64];
        for (int i = 0; i < 16; i++)
            w[i] = (uint32_t(data_[i*4]) << 24) | (uint32_t(data_[i*4+1]) << 16) |
                   (uint32_t(data_[i*4+2]) << 8) | uint32_t(data_[i*4+3]);
        for (int i = 16; i < 64; i++)
            w[i] = sig1(w[i-2]) + w[i-7] + sig0(w[i-15]) + w[i-16];
        uint32_t a = h_[0], b = h_[1], c = h_[2], d = h_[3];
        uint32_t e = h_[4], f = h_[5], g = h_[6], h = h_[7];
        for (int i = 0; i < 64; i++) {
            uint32_t t1 = h + ep1(e) + ch(e, f, g) + k[i] + w[i];
            uint32_t t2 = ep0(a) + maj(a, b, c);
            h = g; g = f; f = e; e = d + t1;
            d = c; c = b; b = a; a = t1 + t2;
        }
        h_[0] += a; h_[1] += b; h_[2] += c; h_[3] += d;
        h_[4] += e; h_[5] += f; h_[6] += g; h_[7] += h;
    }

    std::array<uint32_t, 8> h_;
    uint8_t data_[64];
    size_t datalen_;
    uint64_t bitlen_;
};
